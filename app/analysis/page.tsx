...
export default function AnalysisPage() {
    ...
    return {
      ...
      layout: {
        ...
        annotations: [
          ...
          // Amplitude level labels — Source (green)
          {
            x: selMin, y: results.meanPeakSrc, xanchor: 'left' as const,
            text: `  Src peak: ${results.meanPeakSrc.toFixed(1)}`,
            showarrow: false, font: { color: '#27ae60', size: 10, family: 'monospace' },
            bgcolor: 'rgba(255,255,255,0.8)',
          },
          {
            x: selMin, y: results.meanTroughSrc, xanchor: 'left' as const,
            text: `  Src trough: ${results.meanTroughSrc.toFixed(1)}`,
            showarrow: false, font: { color: '#27ae60', size: 10, family: 'monospace' },
            bgcolor: 'rgba(255,255,255,0.8)',
          },
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const row: Record<string, unknown> = {
        ...
        period_t_resp: results.periodTResp,
        frequency_f_resp: results.frequencyFResp,
  ...
  const handleDownloadCSV = useCallback(() => {
    ...
    const rows = [
      ...
      ['Source Period T (s)', results.periodT.toFixed(4)],
      ['Response Period T (s)', results.periodTResp != null ? results.periodTResp.toFixed(4) : 'N/A'],
      ['Source Frequency f (Hz)', results.frequencyF.toFixed(6)],
      ['Response Frequency f (Hz)', results.frequencyFResp != null ? results.frequencyFResp.toFixed(6) : 'N/A'],
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <section className="space-y-6">
        ...
        <button
          ...
          disabled={loading || (inputMode === 'one' ? !aioFile : (!c80File || !srcFile))}
            ...
            <table className="w-full text-sm border border-[var(--border)]">
              ...
              <tbody>
                {[
                  ...
                  [t('analysis.periodTSrc'), `${results.periodT.toFixed(4)} s`],
                  [t('analysis.periodTResp'), results.periodTResp != null ? `${results.periodTResp.toFixed(4)} s` : '—'],
                  [t('analysis.frequencyFSrc'), `${results.frequencyF.toFixed(6)} Hz`],
                  [t('analysis.frequencyFResp'), results.frequencyFResp != null ? `${results.frequencyFResp.toFixed(6)} Hz` : '—'],
