...
const DISPLAY_COLS: { key: keyof Analysis; label: string; fmt?: (v: unknown) => string; editable?: boolean }[] = [
  ...
  { key: 'period_t', label: 'Src Period (s)', fmt: v => Number(v).toFixed(2), editable: true },
  { key: 'period_t_resp', label: 'Resp Period (s)', fmt: v => v != null ? Number(v).toFixed(2) : '—', editable: true },
  { key: 'frequency_f', label: 'Src f (Hz)', fmt: v => Number(v).toFixed(6), editable: true },
  { key: 'frequency_f_resp', label: 'Resp f (Hz)', fmt: v => v != null ? Number(v).toFixed(6) : '—', editable: true },
